...
class HomeController extends Controller
{
    ...
    public function index(BasicSettingsProvider $basic_settings)
    {
        ...
        return view('frontend.index',compact('page_title','section','page_section'));
    }
    ...
    {
        $page_section = SetupPage::where('slug', 'team')->with(['sections' => function($q){
            $q->where('status',true);
        }])->first();
        if($page_section->status == false) return redirect()->route('index');
        $page_title = setPageTitle(__("Team"));

        return view('frontend.pages.team',compact('page_title','page_section'));
    }

    public function teamDetailsView($id)
    {
        $page_title = setPageTitle(__("Team Details"));
        $lang = selectedLang();
        ...
        $team_member = $team->value->items->$id;

        return view('frontend.pages.team-details',compact('page_title','team_member','lang','default'));
    }
    public function scheduleDetailsView($id)
    {
        $page_title = setPageTitle(__("Schedule Details"));
        $schedule = DailySchedule::findOrfail($id);
        return view('frontend.pages.schedule-details',compact('page_title','schedule'));
    }
    public function galleryView()
    {
        $page_section = SetupPage::where('slug', 'gallery')->with(['sections' => function($q){
            $q->where('status',true);
        }])->first();
        if($page_section->status == false) return redirect()->route('index');
        $page_title = setPageTitle(__("Gallery"));
        return view('frontend.pages.gallery',compact('page_title','page_section'));
    }
    public function blogView()
    {
        ...
        return view('frontend.pages.blog',compact('page_title','page_section'));
    }
